  ...
  return (
    <div className="min-h-screen bg-[#EEF2F6] flex text-[#1E293B] font-sans selection:bg-teal-500/20 selection:text-teal-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0B0F17] text-[#E2E8F0] border-r border-[#1E2536] flex flex-col justify-between transition-transform duration-300 ${
        ...
        <div>
          <div className="p-4 border-b border-[#1E2536] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-teal-500 to-cyan-500 flex items-center justify-center text-white font-black text-sm shadow-[0_4px_12px_rgba(20,184,166,0.35)]">
                AD
              </div>
              <div>
        ...
        <div className="p-4 border-t border-[#1E2536] bg-[#070A10] space-y-3">
          <div className="flex items-center justify-between text-xs font-mono">
            <div>
              <span className="text-white font-bold block">@{user?.username || 'admin'}</span>
              <span className="text-[10px] text-emerald-400">JWT Signed Active</span>
            </div>
            <span className="px-2 py-0.5 rounded bg-slate-800 text-teal-400 text-[10px] font-bold">
              OPS
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-extrabold text-sm sm:text-base text-[#0F172A] tracking-tight uppercase">
                {sidebarMenuItems.find(i => i.id === activeTab)?.label || 'CONTROL DESK'}
              </h2>
              <p className="text-[11px] text-[#64748B] font-mono">
                Real-time MongoDB Atlas & SSE Live Broadcast
              </p>
            </div>
          </div>
          ...
          {activeTab === 'REGISTRATIONS' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                {[
                  { label: 'ALL APPLICATIONS', value: `${rosterTeams.length}`, tag: 'LIVE QUEUE' },
                  { label: 'SOLO REQUESTS', value: `${rosterTeams.filter(t => t.matchType === 'SOLO').length}`, tag: 'SOLO' },
                  { label: 'DUO REQUESTS', value: `${rosterTeams.filter(t => t.matchType === 'DUO').length}`, tag: 'DUO' },
                  { label: 'SQUAD REQUESTS', value: `${rosterTeams.filter(t => (t.matchType || 'SQUAD') === 'SQUAD').length}`, tag: 'SQUAD' }
                ].map((m, i) => (
              ...
              <div className="bg-white border border-[#E2E8F0] rounded-3xl p-5 sm:p-6 shadow-xs space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-[#F1F5F9] border border-[#CBD5E1]">
                    {[
                      { id: 'ALL', label: `All (${rosterTeams.length})` },
                      { id: 'SOLO', label: `Solo (${rosterTeams.filter(t => t.matchType === 'SOLO').length})` },
                      { id: 'DUO', label: `Duo (${rosterTeams.filter(t => t.matchType === 'DUO').length})` },
                      { id: 'SQUAD', label: `Squad (${rosterTeams.filter(t => (t.matchType || 'SQUAD') === 'SQUAD').length})` },
                    ].map((tab) => (
                      <button
                        key={tab.id}
                        onClick={() => {
                ...
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                  {[
                    ...
                    { id: 'PENDING', label: `Pending (${pendingRostersCount})`, highlight: pendingRostersCount > 0 },
                  ...
                  {filteredTeams.length === 0 ? (
                    <div className="text-center py-16 px-4 bg-[#F8FAFC] rounded-3xl border border-[#E2E8F0] space-y-3">
                      ...
                      <p className="text-xs text-[#64748B] max-w-md mx-auto font-mono">
                        When players register via "JOIN NOW" on the website, their full roster and passcode appear here in real time.
                      </p>
                    </div>
                  ) : (
